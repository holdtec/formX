...
为了保证企业级复杂业务表格在频繁交互下的高可用性与正确性，`formx-core` 在底层内置了多项精密控制机制，了解这些原理有助于开发出更健壮的配置方案。

### A. Kahn 拓扑排序重算机制 (DAG)
在传统的响应式系统中，字段变化会触发事件递归分发。如果在复杂的单据中存在菱形依赖（例如：修改 `单价` ➜ 行金额重算 ➜ 折扣重算 ➜ 全局汇总），普通的递归会导致同一个字段被多次触发计算，甚至在界面上出现闪烁（即 Glitch 毛刺现象）。

`formx-core` 的解决机制是：
1. **依赖收集**：引擎在 `initialize` 时会解析所有表达式，构建一张完整的有向无环图 (Dependency Graph)。
2. **拓扑排序**：当 `A` 发生变化时，引擎通过 `getExecutionOrder('A')` 算法对**受影响的子图**进行一次拓扑排序。
3. **顺序计算**：获得诸如 `['items.0.amount', 'subTotal', 'finalAmount']` 的一维顺序列表后，引擎按照该队列顺序**线性依次迭代**计算各节点最新值。
4. **效果**：保证每个受影响字段在此次事务中**有且仅被计算一次**，从而达到最高的计算效率，且天然规避了毛刺问题。

### B. 递归熔断保护 (Recursion Guard)
虽然引擎在启动时会进行循环依赖检测（`detectCycle`），但用户在复杂的业务逻辑（如通过 Store 订阅器异步向表单写值）中仍可能诱发隐蔽的无限循环环路。
* **熔断机制**：`setValue` 入口内置了全局计数器。单次调用链的递归深度一旦突破 `50` 层（可配），引擎将立刻**强制熔断**，跳出当前计算回路，并在 Console 中打印致命级 Warning 警告，避免整个浏览器进程锁死或闪退。

### C. 浮点数震荡保护 (Epsilon Check)
表单经常涉及汇率、折扣、税率等浮点数计算。JavaScript 基于 IEEE 754 标准，存在诸如 `0.1 + 0.2 !== 0.3` 精度缺陷。若表单计算公式为 `A = B + C`，当值变动从 `0.29999999` 变回 `0.3` 时，极其容易引起不必要的链式重算和状态抖动。
* **保护机制**：引擎在进行新旧值对比（`isValueEqual`）时内置了容差检测：
  ```typescript
  if (typeof a === 'number' && typeof b === 'number') {
     return Math.abs(a - b) < Number.EPSILON * 1000;
  }
  ```
  在微小精度误差范围内，引擎会判定值未发生实质变动，直接拦截更新，极大保护了长链路计算下的系统稳定性。

### D. 无效值安全回落网络 (Safe Fallback)
企业单据输入过程中，用户经常清除输入框（产生空串 `""`）或处于未填状态（`null`/`undefined`），或者从后端拉取到无效脏数据（如 `"NA"`、`"N/A"` 等）。如果计算公式直接对其进行乘除，极易在页面上产生 `NaN` 或抛出运行时 Null 指针错误导致页面白屏。
* **防护机制**：自研的表达式引擎在变量取值阶段，会自动对获取的值进行过滤和净化：
  - 非数组变量若为 `undefined` / `null` / `NaN`，或者为非有效数字的字符串（包括空串与带有前后空格的脏文本），都会被**无感且安全地回落为数字 0** 参与四则运算。
  - 这保证了计算链在表单未完全填好时也能处于合理的计算状态，避免前端界面出现 `NaN` 等粗糙体验。
